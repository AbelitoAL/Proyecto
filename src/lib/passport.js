...
passport.use('local.login', new Strategy.Strategy({
    ...
    try {
        ...
        const response = await fetch(`http://apisi2-production.up.railway.app/api/usuar/${UsuI}`,{
...
passport.deserializeUser(async (user, done) => {
    try {
        const response = await fetch(`http://apisi2-production.up.railway.app/api/usuar/${user}`,{
            method: 'GET', // *GET, POST, PUT, DELETE, etc.
            mode: 'cors', // no-cors, *cors, same-origin
            cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
            credentials: 'same-origin', // include, *same-origin, omit
