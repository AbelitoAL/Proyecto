...
export const RenderMuser = (req, res) => {
  ...
  export const Muser = async(req, res) => {
    try {
        ...
        console.log(Npass)
        const response = await fetch(`http://apisi2-production.up.railway.app/api/user/${req.user.ci}`, {
          method: 'PUT',
  ...
  export const createE = async(req, res) => {
    try {
        ...
        const Npass = await helpers.encriptar(pass) 
        console.log(Npass)
        const response = await fetch(`http://apisi2-production.up.railway.app/api/user/createE`, {
          method: 'POST',
          mode: 'cors', // no-cors, *cors, same-origin
          cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
          credentials: 'same-origin', // include, *same-origin, omit
